...
# ===== 修改导入方式，解决 ModuleNotFoundError =====
# 把插件根目录加入 sys.path，这样就可以用 `from utils.text_to_image import text_to_image` 之类的绝对导入
_PLUGIN_DIR = Path(__file__).parent.resolve()
if str(_PLUGIN_DIR) not in sys.path:
    sys.path.insert(0, str(_PLUGIN_DIR))

# 现在用“基于插件目录的绝对导入”，不再使用 .相对导入
...
class MyPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # 检查扩展功能是否可用
        try:
            from core.expansion import ExpansionHandle  # 这里也改为绝对导入
        ...
        try:
            ...
            if self.max_blacklist_duration and duration > self.max_blacklist_duration:
                yield event.plain_result(
                    f"时长不能超过最大限制: {self.max_blacklist_duration}秒"
                )
                return
